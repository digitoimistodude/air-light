...
function a11yDropdownMenuKeyboardNavigation(items, focusableElements) {
  focusableElements.forEach((item) => {
      ...
      if (
        ...
        if (thisElement.parentNode.previousElementSibling) {
          ...
          if (previousItem.querySelector('.sub-menu')) {
            ...
            const previousItemDropdown =
              previousItem.querySelector('.sub-menu');
            ...
            previousItem
              ...
              .setAttribute(
                'aria-label',
                `${air_light_screenReaderText.expand_for} ${
                  previousItem.querySelector('.dropdown-item').innerText
                }`
              );
    ...
    addMultipleEventListeners(item, ['click', 'keydown', 'keypress'], (e) => {
      ...
      const firstDropdown =
        thisElement.parentNode.parentNode.parentNode.querySelector('.sub-menu');
      const thisDropdown = thisElement.nextElementSibling;
      const dropdownToggleButton =
        thisElement.parentNode.parentNode.parentNode.querySelector(
          '.dropdown-toggle'
        );
      ...
      if (e.key === 'Enter' || e.type === 'click') {
        ...
        const linkLabel =
          thisElement.parentNode.querySelector('.dropdown-item').innerText;
        ...
        if (thisElement.getAttribute('aria-expanded') === 'false') {
          ...
          thisElement.setAttribute(
            'aria-label',
            `${air_light_screenReaderText.collapse_for} ${linkLabel}`
          );
        } else {
          ...
          thisElement.setAttribute(
            'aria-label',
            `${air_light_screenReaderText.expand_for} ${linkLabel}`
          );
        }
      ...
      if (e.key === 'Escape') {
        ...
        // eslint-disable-next-line camelcase, no-undef
        dropdownToggleButton.setAttribute(
          'aria-label',
          `${air_light_screenReaderText.expand_for} ${linkLabel}`
        );
      ...
      switch (e.code) {
        ...
        case 'ArrowUp':
          ...
          if (thisElement.tagName === 'A' || thisElement.tagName === 'BUTTON') {
            ...
            const previousLink =
              thisElement.parentNode.previousElementSibling.querySelector('a');

            // Get .dropdown-toggle element
            const previousToggle =
              thisElement.parentNode.previousElementSibling.querySelector(
                '.dropdown-toggle'
              );
        ...
        case 'ArrowDown':
          ...
          if (
            ...
            // Get the next link
            const nextLink =
              thisElement.parentNode.nextElementSibling.querySelector('a');

            // Get .dropdown-toggle element
            let nextToggle =
              thisElement.parentNode.nextElementSibling.querySelector(
                '.dropdown-toggle'
              );
