...
if ( ! function_exists( 'air_entry_footer' ) ) {
	...
	function air_entry_footer() {
		...
		edit_post_link(
			sprintf( _x( 'Edit %s', '%s: Name of current post', 'air' ), the_title( '<span class="screen-reader-text">"', '"</span>', false ) ),
			'<p class="edit-link">',
			'</p>'
		);
	}
		...
		<li id="li-comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div id="comment-<?php comment_ID(); ?>">
				...
				<?php if ( '0' === $comment->comment_approved ) : ?>
					<p><em><?php esc_html_e( 'Your comment is awaiting approval.', 'air' ); ?></em></p>
				<?php endif; ?>
				...
				<p class="comment-time">
					<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
						...
						<time><?php printf( __( '%1$s at %2$s', 'air' ), get_comment_date(), get_comment_time() ); ?></time>
					</a>
				</p>
				...
				edit_comment_link( __( '&mdash; Edit', 'air' ), ' ', '' ) ?>
